...
/**
 * Abstract base for classes that provide an application certificate and a
 * matching key pair.
 *
 * <p>
 * Certificate providers should be created through the static factory methods in
 * this class. After
 * creation, the {@link #load()} method must be invoked on them before the
 * certificate and key pair
 * can be retrieved.
 *
 * <p>
 * A fallback provider can be set which will be invoked automatically, if this
 * provider fails to
...
public abstract class CertificateProvider {
  ...
  /**
   * Loads the key pair and certificate into memory, so they can be retrieved from
   * this provider.
   *
   * <p>
   * This method must be called before either {@link #getCertificate()} or
   * {@link #getKeyPair()}.
   *
   * @throws GeneralSecurityException If this provider or its fallback can't
   *                                  provide the key pair and
   ...
   *
   * <p>
   * Aside from possible log entries or other side effects, usage of the fallback
   * happens
   * transparently to the caller during {@link #load()}. Its key pair and
   * certificate will the
   ...
   * {@link #setKeyPairAndCertificate(KeyPair, X509Certificate)} to set both
   * objects.
   *
   * <p>
   * If it fails with a {@link GeneralSecurityException} and a fallback has been
   * set on this provider
   * the fallback will be automatically loaded and its results stored in this
   * provider.
   *
   * @throws GeneralSecurityException if the provider can't provide the key pair
   *                                  or certificate for
   *                                  any reason.
   */
   ...
   *
   * <p>
   * This method is called by {@link CertificateProvider#load()} in case
   * {@link #loadInternal()}
   * failed and the fallback has succeeded. It allows an implementation to store
   * the fallback's
   ...
   *
   * @throws NullPointerException if either <i>keyPair</i> or <i>certificate</i>
   *                              are
   *                              {@code null}.
   */
  ...
  /**
   * Creates a new certificate provider which retrieves keys and certificate from
   * a key store stored
   * in a file.
   *
   * <p>
   ...
   * @param keyStoreType     The type of the key store. If its the default
   *                         "PKCS12", consider using
   *                         {@link #fromKeyStore(File, String, String, String)},
   *                         instead.
   ...
   *
   * <p>
   * The key store is assumed to have the type "PKCS12". To specify another type,
   * please use
   ...
   * @param alias    The alias under which the private key and certificate are
   *                 stored.
  ...
  /**
   * Creates a new provider which generated a certificate with the given subject
   * info.
   *
   * @param commonName         The common name (CN) of the certificate's subject.
   *                           Makes up part of its
   *                           distinguished name (DN).
   * @param organization       The organization (O) of the certificate's subject.
   *                           Makes up part of its
   *                           distinguished name (DN).
   * @param organizationalUnit The organizational unit (OU) of the certificate's
   *                           subject. Makes up
   *                           part of its distinguished name (DN).
   * @param countryCode        The country code (C) of the certificate's subject.
   *                           Makes up part of its
   *                           distinguished name (DN).
   * @param locality           The locality (L) of the certificate's subject.
   *                           Makes up part of its
   *                           distinguished name (DN).
   * @param state              The state (ST) of the certificate's subject. Makes
   *                           up part of its
   *                           distinguished name (DN).
   * @param applicationUri     The application URI (CN) of the certificate's
  ...
  /**
   * Creates a simple provider which holds the given keys and certificate in
   * memory for later
   * retrieval.
   *
   * @param keyPair     The key pair.
